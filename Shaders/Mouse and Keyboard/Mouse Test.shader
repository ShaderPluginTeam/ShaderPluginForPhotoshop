<Shader>
	...
	<CommonCode>// Smaller Number Printing - @P_Malin
...
void main()
{
    ...
    vec2 vFontSize = vec2(16) * vec2(Aspect, 1.0);
    vec2 cUV = (f_UV - vec2(0.5)) * vFontSize + vec2(0.5);
    vec4 mUV1 = iMouse / iViewSize.xyxy;
    vec2 mUV2 = iMouseCoords / iViewSize;

	// Pressed and current coords locations
    FragColor.rgb  = step(distance(f_UV,    (mUV1.xy)), 0.01) * vec3(1, 1, 0);
    FragColor.rgb += step(distance(f_UV, abs(mUV1.zw)), 0.01) * vec3(0, 1, 1);
    FragColor.rgb += step(distance(f_UV, abs(mUV2)),   0.005) * vec3(1, 0, 1);
    
    // iMouse.xy
    FragColor.rgb += PrintValue(cUV + vec2(0,-6), mUV1.x, 1, 5) * vec3(1, 0, 0);
    FragColor.rgb += PrintValue(cUV + vec2(0,-4), mUV1.y, 1, 5) * vec3(0, 1, 0);
    
    // iMouse.zw
    FragColor.rgb += PrintValue(cUV + vec2(0,-2), mUV1.z, 1, 5) * vec3(0, 0, 1);
    FragColor.rgb += PrintValue(cUV + vec2(0, 0), mUV1.w, 1, 5) * vec3(1, 1, 0);
    
    // iMouseCoords.xy
    FragColor.rgb += PrintValue(cUV + vec2(0, 4), mUV2.x, 1, 5) * vec3(1, 0, 0);
    FragColor.rgb += PrintValue(cUV + vec2(0, 6), mUV2.y, 1, 5) * vec3(0, 1, 0);
