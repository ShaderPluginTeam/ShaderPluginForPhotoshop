...
vec3 crystalsd(vec2 pos, vec2 scale, float jitter, float phase, float seed)
{
    vec3 c0 = cellularNoised(pos, scale, jitter, phase, seed);
    vec3 c1 = cellularNoised(pos, scale, jitter, phase, seed + 23.0);
    c0.x = 1.0 - c0.x;
    c1.x = 1.0 - c1.x;
    if (c0.x &gt; c1.x)
    {
...
void main()
{
	...
	vec3 CrystalsD = crystalsd(f_UV, Scale, Jitter, Phase, Seed);
	
	// Crystals
	FragColor = vec4(CrystalsD.xxx, 1.0);
	
	// Crystals NormalMap
	// FragColor = vec4(CrystalsD.yz * 0.5 + 0.5, 1.0, 1.0);
}</Shader_FS>
		<TextureFilter>
