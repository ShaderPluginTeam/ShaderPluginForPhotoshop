<Shader>
	...
	<CommonCode>float sdfCapsule(vec2 p, float radiusA, float radiusB, float height)
{
...
void main()
{
	...
	vec3 Weave = tileWeave(f_UV, Scale, Count, Width, Smoothness);
	
	// Weave Pattern
	vec3 Pattern = vec3(1.0) - Weave.xxx;

	// Normal Map
	// vec3 Pattern = vec3(Weave.yz * 0.5 + 0.5, 1.0);
