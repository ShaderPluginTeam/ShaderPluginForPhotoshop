...
float Checkerboard(vec2 pos, vec2 scale, vec2 smoothness)
{
...
float Checkerboard45(vec2 pos, vec2 scale, vec2 smoothness)
{
    // based on filtering the checkerboard by Inigo Quilez 
...
void main()
{
	...
	vec2 Smoothness = vec2(0.0);

	float Checkerboard = Checkerboard(f_UV, Scale, Smoothness);
	float Checkerboard45 = Checkerboard45(f_UV, Scale, Smoothness);
	
	// Checkerboard 90*
	// vec3 Pattern = vec3(Checkerboard);
	
	// Checkerboard 45*
	vec3 Pattern = vec3(Checkerboard45);
	
	FragColor = vec4(Pattern, 1.0);
}</Shader_FS>
		<TextureFilter>
