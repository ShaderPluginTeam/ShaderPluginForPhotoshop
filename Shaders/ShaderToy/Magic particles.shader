...
uniform vec2	iImageSize;		// Image Size [vec2, ivec2]
//uniform vec2	iViewSize;		// Viewport Size, also can use iResolution. [vec2, ivec2]
uniform vec2	iResolution;	// Viewport Size, also can use iViewSize. [vec2, ivec2]
uniform vec4	iRandom;		// Random values [0 .. 1]
uniform vec4	iDate;			// Year, Month, Day, Time in seconds
uniform float	iTime;			// Running time (sec)
uniform float	iTimeDelta;		// Frame render time (sec)
...
#endif
const vec2 gen_scale = vec2(0.5, 0.5);                      // To scale the particle positions, not the particles themselves
...
{
	vec2 uv = fragCoord.xy; // vec2 uv = fragCoord.xy / iResolution.xx;
    
    // Background gradient
    vec3 pcolor = vec3(0.0, (0.6 - uv.y)/6.0, (1.0 - uv.y)/5.0);
...
void main()
{
	mainImage(FragColor, f_UV); // f_UV * iResolution
}</Shader_FS>
		<TextureFilter>
